...
console.log('--- ЗАПУСК СКРИПТА (ULTIMATE STABLE VERSION) ---');
...
async function main() {
    ...
    const context = await browser.newContext({ 
        ...
        deviceScaleFactor: 2, // Повышенная плотность для четкости
    ...
    try {
        ...
        console.log(`Текущий заголовок: "${pageTitle}"`);

        // ПРОВЕРКА НА ВЫЛЕТ ИЗ СЕССИИ
        if (currentUrl.includes('esia') || pageTitle.includes('вторизац') || pageTitle.includes('Вход')) {
            console.error('❌ КРИТИЧЕСКАЯ ОШИБКА: state.json не сработал или куки протухли!');
            console.error('Нужно обновить секрет STATE_JSON_B64 новыми данными из браузера.');
            await browser.close();
            process.exit(1);
        }
        ...
        for (const url of links.slice(0, 10)) {
            ...
            try {
                ...
                if (title.toLowerCase().includes('изменени')) {
                    const pdfLink = await p.getAttribute('a[href*=".pdf"], a[href*="/download/"], a[href*="attachment"]', 'href');
                    if (pdfLink) {
        ...
        // Сортировка от старых к новым (чтобы последние стали ID больше)
        ...
        for (const item of foundNews) {
            try {
                ...
                console.log(`Обработка файла: ${prettyTitle}`);

                const pdfResp = await context.request.get(item.url);
                const pdfBuf = await pdfResp.body();
                const b64Pdf = pdfBuf.toString('base64');

                // Рендерим PDF через PDF.js (Scale 3.0 для супер-качества)
                const p = await context.newPage();
                await p.setViewportSize({ width: 1100, height: 1000 });
                await p.setContent(`
                    <html><head>
                        <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js"></script>
                        <style>body{margin:0;background:#fff;padding:10px} canvas{display:block;margin:0 auto 10px auto;}</style>
    ...
    // ОЧИСТКА БАЗЫ САЙТА (оставляем 3 записи)
    try {
